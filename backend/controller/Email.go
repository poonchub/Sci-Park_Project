...
func SendMaintenanceStatusEmail(c *gin.Context) {
	ID := c.Param("id")
	...
	if err := db.First(&request.User, request.UserID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "ไม่พบข้อมูลผู้ใช้งาน"})
		return
	}

	if err := db.First(&request.Room, request.RoomID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "ไม่พบข้อมูลผู้ใช้งาน"})
		return
	}

	if err := db.First(&request.RequestStatus, request.RequestStatusID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "ไม่พบข้อมูลผู้ใช้งาน"})
		return
	}
	...
	if err := db.Where("request_id = ?", ID).First(&task).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			var approval entity.ManagerApproval
			if err := db.Where("request_id = ?", ID).First(&approval).Error; err != nil {
				c.JSON(http.StatusNotFound, gin.H{"error": "ไม่พบข้อมูลการตรวจสอบหรือการทำงาน"})
				return
			}
	...
	updatedAt := request.UpdatedAt.Format("2006-01-02 15:04")
											...
											<tr>
												<td class="column column-1" width="100%%" style="mso-table-lspace: 0pt; mso-table-rspace: 0pt; font-weight: 400; text-align: left; padding-bottom: 5px; padding-top: 5px; vertical-align: top;">
													...
													<table class="heading_block block-3" width="100%%" border="0" cellpadding="10" cellspacing="0" role="presentation" style="mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
														<tr>
															<td class="pad">
																<h1 style="margin: 0; color: #000000; direction: ltr; font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 32px; font-weight: 700; letter-spacing: normal; line-height: 120%%; text-align: left; margin-top: 0; margin-bottom: 0; mso-line-height-alt: 45.6px;">%s</h1>
															</td>
														</tr>
													</table>
												</td>
