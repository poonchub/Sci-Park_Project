...
func GetMaintenanceRequestByID(c *gin.Context) {
	...
	db := config.DB()
	results := db.Preload("User").Preload("Room.Floor").Preload(("Room.RoomType")).Preload("RequestStatus").Preload("Area").Preload("MaintenanceType").Preload("ManagerApproval.User").Preload("MaintenanceTask.User").Preload("MaintenanceImages").First(&request, ID)
	if results.Error != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": results.Error.Error()})
		return
	}
